...
def mdn_select_v2(**kwargs):
    ...
    candidate_scores = {}
    for node in unobserved_nodes:
        candidate_score = np.sum(np.array(entropies)[unobserved_nodes] * distance_weights[node][unobserved_nodes]) - entropies[node]
        normalisation = np.sum(distance_weights[node][unobserved_nodes]) - distance_weights[node][node]
        candidate_scores[node] = candidate_score / normalisation
